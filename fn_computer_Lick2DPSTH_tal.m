function fn_computer_Lick2DPSTH_tal(key,self, rel_data, fr_interval,fr_interval_limit, flag_electric_video, time_resample_bin, self2, self3, self4)

smooth_window_sec=0.2; %frames for PSTH

rel_ROI = (IMG.ROI-IMG.ROIBad) & key;
key_ROI1=fetch(rel_ROI,'ORDER BY roi_number'); % LICK2Dtal.ROILick2DPSTHSpikes
key_ROI2=fetch(rel_ROI,'ORDER BY roi_number'); % LICK2Dtal.ROILick2DPSTHStatsSpikes
key_ROI3=fetch(rel_ROI,'ORDER BY roi_number'); % LICK2Dtal.ROILick2DPSTHBlockSpikes
key_ROI4=fetch(rel_ROI,'ORDER BY roi_number'); % LICK2Dtal.ROILick2DPSTHBlockStatsSpikes
...
R=fetch((EXP2.TrialRewardSize & key),'*','ORDER BY trial'); % - TRACKING.VideoGroomingTrial
Block=fetch((EXP2.TrialLickBlock & key),'*','ORDER BY trial'); % - TRACKING.VideoGroomingTrial
...
[start_file, end_file ] = fn_parse_into_trials_and_get_lickrate_tal (key, frame_rate, fr_interval, flag_electric_video);
...
try
    ...
    idx_first = find([Block.current_trial_num_in_block]==1 & idx_response & idx_regular_temp); % index of the current trail in block==1&not nan& ind of the 01 map of regular reward
...
% we take the session epoch type and number of the current key (one
% iteration) and apply it to the same possition in the each key_ROI(i). we
% are going to use key_ROI for %%%
for i_roi=1:1:size(S,1)
    ...
    %% PSTH
    %spikes is the dff trace of each row in the table
